...
def InitBoundZ(n, m, Bt, z_0, c_theta, r_theta):
    ...
    return z_LB, z_UB, LB_theta, UB_theta


def BoundPreprocessing(n, k, At, y_LB, y_UB, z_LB, z_UB, LB_theta, UB_theta):
...
def BoundTightY(k, n, At, Bt, lambda_t, z_0, c_theta, r_theta, y_LB, y_UB, z_LB, z_UB):
...
def IncrementalVerifierISTA(K, At, Bt, lambda_t, z_0, c_theta, r_theta):
    ...
    def ModelNextStep(model, n, k, At, Bt, lambda_t, z_0, c_theta, r_theta, y_LB, y_UB, z_LB, z_UB):
    ...
    m = len(c_theta)
    # Local variables
    z_LB, z_UB, LB_theta, UB_theta = InitBoundZ(n, m, Bt, z_0, c_theta, r_theta)
    y_LB, y_UB = {}, {}
    ...
    for k in range(1, K):
        ...
        result = ModelNextStep(model, n, k, At, Bt, lambda_t, z_0, c_theta, r_theta, y_LB, y_UB, z_LB, z_UB)
        if result is None:
            return None
        delta[k] = result
