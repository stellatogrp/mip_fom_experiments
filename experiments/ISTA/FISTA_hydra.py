...
def BuildRelaxedModel(K, At, Bt, gamma, lambda_t, y_LB, y_UB, z_LB, z_UB, v_LB, v_UB, x_l, x_u):
    ...
    for k in range(1, K+1):
        model.addConstr(y[k] == At @ v[k-1] + Bt @ x)
...
def BoundTightV(k, At, Bt, gamma, lambda_t, y_LB, y_UB, z_LB, z_UB, v_LB, v_UB, x_LB, x_UB):
    log.info('bound tightening for v')
...
def add_pos_conv_cuts(cfg, k, i, At, Bt, lambda_t, v_LB, v_UB, x_LB, x_UB, v, x, z_out):
    n, m = Bt.shape
    L_hat = jnp.zeros((m + n))
    U_hat = jnp.zeros((m + n))
    ...
    for j in range(n):
        if Ati[j] >= 0:
            L_hat = L_hat.at[j].set(vkminus1_LB[j])
            U_hat = U_hat.at[j].set(vkminus1_UB[j])
        else:
            L_hat = L_hat.at[j].set(vkminus1_UB[j])
            U_hat = U_hat.at[j].set(vkminus1_LB[j])
...
def create_new_pos_constr(cfg, k, i, At, Bt, lambda_t, Iint, lI, h, z, v, x, Lhat, Uhat):
    Ati = At[i]
    Bti = Bt[i]

    vx_stack = gp.hstack([v[k-1], x])
    w = jnp.hstack([Ati, Bti])

    new_constr = 0
    for idx in Iint:
        new_constr += w[idx] * (vx_stack[idx] - Lhat[idx])
    new_constr += (lI - lambda_t) / (Uhat[h] - Lhat[h]) * (vx_stack[h] - Lhat[h])
...
def add_neg_conv_cuts(cfg, k, i, At, Bt, lambda_t, v_LB, v_UB, x_LB, x_UB, v, x, z_out):
    n, m = Bt.shape
    L_hat = jnp.zeros((m + n))
    U_hat = jnp.zeros((m + n))
...
def create_new_neg_constr(cfg, k, i, At, Bt, lambda_t, Iint, uI, h, z, v, x, Lhat, Uhat):
    Ati = At[i]
    Bti = Bt[i]
    ...
    new_constr = 0
    for idx in Iint:
        new_constr += w[idx] * (vx_stack[idx] - Uhat[idx])
    new_constr += (uI + lambda_t) / (Lhat[h] - Uhat[h]) * (vx_stack[h] - Uhat[h])
...
def FISTA_verifier(cfg, A, lambd, t, c_z, x_l, x_u):
    ...
    def ModelNextStep(model, k, At, Bt, gamma, lambda_t, y_LB, y_UB, z_LB, z_UB, v_LB, v_UB, obj_scaling=cfg.obj_scaling.default):
        obj_constraints = []
        ...
        vobj = model.addMVar(n, vtype=gp.GRB.BINARY)
        ...
        if pnorm == 1 or pnorm == 'inf':
            ...
            for i in range(n):
                obj_constraints.append(up[i] <= np.abs(z_UB[k, i] - z_LB[k-1, i]) * vobj[i])
                obj_constraints.append(un[i] <= np.abs(z_LB[k, i] - z_UB[k-1, i]) * (1 - vobj[i]))
        ...
        if cfg.exact_conv_relax.use_in_l1_rel:
            ...
            for i in range(n):
                ...
                if (k, i) in w1 and rel_z_out[i] > 0:
                    Iint, lI, h, L_hat, U_hat = add_pos_conv_cuts(cfg, k, i, At, Bt, lambda_t, v_LB, v_UB, x_LB, x_UB, rel_v, rel_x, rel_z_out)

                    if Iint is not None:
                        log.info(f'new lI constraint added with {(k, i)}')
                        model.addConstr(create_new_pos_constr(cfg, k, i, At, Bt, lambda_t, Iint, lI, h, z, v, x, L_hat, U_hat))

                if (k, i) in w2 and rel_z_out[i] < 0:
                    Iint, uI, h, L_hat, U_hat = add_neg_conv_cuts(cfg, k, i, At, Bt, lambda_t, v_LB, v_UB, x_LB, x_UB, rel_v, rel_x, rel_z_out)

                    if Iint is not None:
                        log.info(f'new uI constraint added with {(k, i)}')
                        model.addConstr(create_new_neg_constr(cfg, k, i, At, Bt, lambda_t, Iint, uI, h, z, v, x, L_hat, U_hat))
    ...
    for k in range(1, K_max+1):
        ...
        ax.set_yscale('log')
        ax.set_title(rf'FISTA VP, $m={cfg.m}$, $n={cfg.n}$')
