...
def model_and_solve(cfg, K_max, A, c, t, utilde_LB, utilde_UB, u_LB, u_UB, v_LB, v_UB, x_LB, x_UB, momentum=False, beta_func=None, u_val=None, v_val=None):
    ...
    x = model.addMVar(m, lb=x_LB, ub=x_UB)
    utilde = model.addMVar(n_var_shape, lb=utilde_LB[:K_max+1], ub=utilde_UB[:K_max+1])
    u = model.addMVar(n_var_shape, lb=u_LB[:K_max+1], ub=u_UB[:K_max+1])
    v = model.addMVar(m_var_shape, lb=v_LB[:K_max+1], ub=v_UB[:K_max+1])
    ...
    return model.objVal / cfg.obj_scaling, model.Runtime, u.X, v.X, x.X
...
def LP_run(cfg, A, c, t, u0, v0):
    ...
    for k in range(1, K_max+1):
        ...
        result, time, u_val, v_val, x_val = model_and_solve(cfg, k, A, c, t, utilde_LB, utilde_UB, u_LB, u_UB, v_LB, v_UB, x_LB, x_UB, momentum=momentum, beta_func=beta_func, u_val=u_val, v_val=v_val)

        x_out = x_out.at[k-1].set(x_val)
        log.info(result)
        log.info(x_val)
